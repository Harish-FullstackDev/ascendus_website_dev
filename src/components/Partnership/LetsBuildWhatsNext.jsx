...
export default function LetsBuildWhatsNext() {
    return (
        ...
        >
            {/* Figma: a 313px intro column beside a 1003px card track. The intro
                track is capped rather than fixed so the cards absorb the extra
                width on wider screens. */}
            <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,313px)_minmax(0,1fr)] gap-10 lg:gap-8 lg:items-center">
                <motion.div
                ...
                >
                    ...
                    <h2 className="text-2xl sm:text-[30px] font-semibold tracking-[-0.75px] text-[#0f172a] leading-[36px]">
                        Let&apos;s Build What&apos;s Next
                    </h2>
                    <p className="text-sm text-[#475569] leading-[19.5px]">
                    ...
                    <Link
                        ...
                        className="group mt-3 inline-flex items-center gap-2 rounded-[10px] bg-[#0b1c34] px-5 py-2.5 text-sm font-semibold text-white shadow-[0px_1px_1px_rgba(0,0,0,0.05)] transition-colors duration-300 hover:bg-[#0a3a52]"
                    >
                        Become a Partner
                        <Image
                            src={arrowRightIcon}
                ...
                {/* Five compact cards across a single track (Figma ~169px each,
                    14px gutters). They fall to three and then two columns rather
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
                    {CARDS.map((card) => (
                        <motion.article
                            ...
                            className="flex h-full flex-col rounded-[12px] border border-[rgba(226,232,240,0.8)] bg-white p-5 transition-shadow duration-300 hover:shadow-[0px_6px_18px_rgba(10,58,82,0.1)]"
                        >
                            {/* Figma alternates 40px/rounded-8 and 36px/rounded-12
                                tiles across these five cards; normalised to one
                                treatment so the row reads as a set. */}
                            ...
                            <h3 className="mt-3 text-sm font-semibold text-[#0f172a] leading-[16.5px]">{card.title}</h3>
                            <p className="mt-2 text-xs text-[#64748b] leading-[16.5px]">{card.description}</p>
                        </motion.article>
                    ))}
                </div>
            </div>
